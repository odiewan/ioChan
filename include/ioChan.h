...
#define PWM_DC_PER_PWM_ENG    2.125
...
enum ioChanTypeEnum {
  ...
  IO_TYPE_AIN_31V_3V3,            //--- 10: 0-31v scaled to 3.3v 10bit Analog
...
class ioChannel {
  public:
    ...
    ioChannel();
    void initGains();
    void constructorCore(ioChanTypeEnum nType, int nPin, int *eValPtr, float *eValPtrFlt);

    ioChannel(ioChanTypeEnum nType, int nPin, int *eValPtr);
    ioChannel(ioChanTypeEnum nType, int nPin, float* eValPtrFlt);
